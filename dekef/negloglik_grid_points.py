...
class NegLogLikGridPoints:
	...
	def __init__(self, data, grid_points, base_density,
		...
		if grid_points.shape[1] != data.shape[1]:
			raise ValueError('The dimensions of grid_points and data do not match.')
	...
	def grad_logpar_batchmc(self, coef, batch_size, tol_param, compute_grad=True, print_error=False):
		...
		"""
        Approximates the partition function and the gradient of the log-partition function at coef
        with the kernel functions centered at grid_points.
        The approximation method used is the batch Monte Carlo method.
        Terminate the sampling process until the relative difference of
        two consecutive approximations is less than tol_param.

        Let Y_1, ..., Y_M be random samples from the base density.
        The partition function at coef is approximated by
        (1 / M) sum_{j=1}^M exp ( sum_{i=1}^n coef[i] k (X_i, Y_j) ),
        and the gradient of the log-partition function at coef is approximated by
        (1 / M) sum_{j=1}^M k (X_l, Y_j) exp ( sum_{i=1}^N coef[i] k (X_i, Y_j) - A(coef)), for all l = 1, ..., m,
        ...
        -------
        float
            The approximation of the partition function at coef.
		...
		if self.grid_points.shape[0] != coef.shape[0]:
			raise ValueError('The lengths of grid_points and coef do not match.')
	...
	def grad_logpar_batchmc_se(self, coef, batch_size, tol_param, compute_grad=True, print_error=False):
        ...
        ----------
        coef : numpy.ndarray
            The array of coefficients at which the partition function and the gradient of the log-partition function
            are approximated.
	...
	def coef(self, data, lambda_param, optalgo_params, batchmc_params, batch_mc=True, print_error=True):
		...
		"""
        Returns the solution that minimizes the penalized negative log-likelihood loss function
        with the basis functions centered at grid_points.
	...
	def penalized_optlambda(self, lambda_cand, k_folds, print_error, optalgo_params,
							batchmc_params, save_dir, save_info=False, batch_mc=True):
		...
		if np.any(lambda_cand < 0.):
			raise ValueError("There exist negative elements in lambda_cand. Please modify.")
