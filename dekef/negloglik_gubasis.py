...
def negloglik_gubasis_grad_logpar_batchmc(data, kernel_function, base_density, coef, batch_size, tol_param,
    ...
    References
    ----------
    Gu, Chong, and Chunfu Qiu. 1993. “Smoothing Spline Density Estimation: Theory.” Annals of Statistics 21 (1): 217–34.
    Gu, Chong. 1993. “Smoothing Spline Density Estimation: A Dimensionless Automatic Algorithm.” Journal of the American
...
def negloglik_gubasis_grad_logpar_batchmc_se(data, kernel_function, base_density, coef, batch_size, tol_param,
                                             normalizing_const_only=False, print_error=False):
...
def negloglik_gubasis_coef(data, kernel_function, base_density, lambda_param, optalgo_params, batchmc_params,
    ...
    using the basis functions in Gu and Qiu (1993) and Gu (1993).
...
def negloglik_gubasis_loss_function(data, new_data, kernel_function, base_density, coef, batchmc_params,
    ...
    phi_1, ..., phi_N are the basis functions in Gu and Qiu (1993) and Gu (1993), and Y_1, ..., Y_n are new_data.
    ...
    if batch_mc:
        ...
        mc_output1 = negloglik_gubasis_grad_logpar_batchmc(
            data=data,
            kernel_function=kernel_function,
            base_density=base_density,
            coef=coef,
        ...
        mc_output1 = negloglik_gubasis_grad_logpar_batchmc_se(
            data=data,
            kernel_function=kernel_function,
            base_density=base_density,
            coef=coef,
...
def negloglik_gubasis_penalized_optlambda(data, kernel_function, base_density,
    ...
    The basis functions of the natural parameter are k (X_1, .), ..., k (X_n, .), the ones from Gu and Qiu (1993)
    and Gu (1993).
